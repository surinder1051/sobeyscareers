...
class Slide {
	...
	public function shortcode( $atts, $content = null ) {
		...
		$slide_class  .= $box_alignment && true === $text_box ? " box_align_$box_alignment" : '';
		...
		$alt          = $thumbnail_id ? trim( wp_strip_all_tags( get_post_meta( $thumbnail_id, '_wp_attachment_image_alt', true ) ) ) : '';
		...
		if ( $video_url ) {
			...
			if ( preg_match( '/vimeo/', $url_parts['host'] ) ) {
				$video_id   = preg_replace( '/\/(?:video\/)*/', '', $url_parts['path'] );
				$video_type = 'vimeo';
			} elseif ( preg_match( '/youtu\.*be/', $url_parts['host'] ) ) {
				$video_id = isset( $url_parts['query'] ) ? ( strpos( $url_parts['query'], '&' ) ? substr( $url_parts['query'], 0, strpos( $url_parts['query'], '&' ) ) : $url_parts['query'] ) : $url_parts['path'];
				$video_id = preg_replace( '/(\/(?:embed\/)*|v=)/', '', $video_id );
			}
		}
	 ...
	 */
	public function run() {
		add_shortcode( 'fp_slide', array( $this, 'shortcode' ) );
	}
}

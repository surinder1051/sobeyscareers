		...
		switch ( $object['type'] ) {
			...
			case 'date_picker':
				if ( isset( $object['date_format'] ) && ! isset( $object['return_format'] ) ) {
					...
					if ( isset( $settings->format ) && '' !== $settings->format && isset( $value ) && ! empty( $value ) ) {
			...
			case 'image':
				...
				break;
			case 'file':
				$url  = self::get_file_url_from_object( $object );
				$file = pathinfo( $url );
				if ( is_string( $content ) && is_array( $file ) ) {
					switch ( $settings->display ) {
						case 'name':
							$content = isset( $file['filename'] ) && ! empty( $file['filename'] ) ? $file['filename'] : '';
							break;
						case 'basename':
						...
						case 'ext':
							...
							break;
						case 'title':
							$content = isset( $object['value']['title'] ) ? $object['value']['title'] : '';
							break;
						case 'description':
							$content = isset( $object['value']['description'] ) ? $object['value']['description'] : '';
							break;
						case 'alt':
							$content = isset( $object['value']['alt'] ) ? $object['value']['alt'] : '';
							break;
						case 'caption':
							$content = isset( $object['value']['caption'] ) ? $object['value']['caption'] : '';
							break;
						default:
							$content = $url;
							break;
	...
	static public function relational_field( $settings, $property ) {
		...
		if ( $bail_out ) {
			...
			if ( 'user' == $object['type'] ) {
				...
				$list_class_name     = sanitize_html_class( $settings->list_class_name, '' );
				$list_item_tag       = 'div';
				$list_tag            = $settings->list_type;
				$list_item_separator = '';
				...
				$list_tag_open  = '<' . $list_tag . ( empty( $list_class_name ) ? '' : ' class="' . $list_class_name . '"' ) . '>';
				$list_tag_close = "</$list_tag>";
				$content_items  = array();
				...
				$content = $list_tag_open . join( $list_item_separator, $content_items ) . $list_tag_close;

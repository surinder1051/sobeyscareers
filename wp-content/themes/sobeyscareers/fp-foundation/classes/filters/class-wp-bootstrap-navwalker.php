...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$output .= "{$n}{$indent}<ul$class_names $labelledby role=\"menu\" aria-label=\"Sub menu navigation list\" >{$n}";
		}
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( ! empty( $icon_class_string ) ) {
				...
				$icon_html = '<i class="' . esc_attr( $icon_class_string ) . "\" $icon_class_font_size aria-hidden=\"true\"></i> ";
			}
			...
			if ( in_array( 'sr-only', $linkmod_classes, true ) ) {
				...
				$keys_to_unset = array_keys( $linkmod_classes, 'sr-only' ); // phpcs:ignore.
			...
			if ( isset( $args->vertical_expand_buttons ) && in_array( 'dropdown', $classes ) ) { // phpcs:ignore.
				...
				$item_output .= '<button class="bs-nav-expander" aria-controls="menu-item-' . $item->ID . '" data-open-icon="' . $args->vertical_expand_buttons['open-icon'] . '" data-close-icon="' . $args->vertical_expand_buttons['close-icon'] . '" aria-label="' . __( 'Expand menu items', 'fp-foundation' ) . '">';
...
if ( ! function_exists( 'prefix_modify_nav_menu_args' ) ) {
	/**
	 * Force all menus go go through the foundation nav walker.
	 *
	 * @param array $args are the pre-defined menu arguments.
	 * @see WP_Bootstrap_Navwalker::()
	 *
	 * @return array
	 */
